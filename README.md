...
- 点击文件进右侧停靠面板看 **diff 视图**（默认）：完整文件着色渲染（删除红 /
  ...
  **未跟踪文件**（从未提交）没有基线版本，默认直接进原文视图，切到 diff 也按
  "整文件新增"全绿着色而非空提示
...
- 点开**右侧停靠面板**列出当前会话运行中的后台任务（与文件预览共用停靠位、
  互斥打开，对话可继续；只列进行中，任务源是官方
...
- ✕ 按钮 / Esc 均可关闭，左缘可拖宽；任务结束即从列表消失（只列进行中）
...
- **链接令牌鉴权**：插件自带网关（默认端口 3090，**面板内可改**，1-65535，保存后
  自动按新端口重启）监听 0.0.0.0，`?k=<令牌>` → HttpOnly Cookie → 302 到 GUI；
  无/错令牌一律 404，Cookie 过期即失效
- **默认每次启动都是关闭的**：DSH 重启后网关不再自动恢复开启；勾选 dsh-kit 设置卡
  「重启后保留开启」（保存后下次启动生效）才沿用上次的启用状态（同一令牌，已授权
  设备不掉线）
- **手动轮换令牌**：页内「刷新链接」按钮作废当前链接并生成新链接，旧链接与已授权
  设备即时失效；启停不再自动轮换，重启/重开沿用同一令牌
- 局域网与远程双通道：本地每个 IPv4 一条 `http://<ip>:<端口>/?k=…`，配置远程域名后
  附 `https://<域名>/?k=…`（配 frp + caddy 模板，见 `scripts/vps/`），扫码即开；
  **远程链接不展示地址与二维码**（含访问令牌，防截屏/旁观泄露），只留复制按钮
...
- 免 key 引擎链按优先级自动故障转移：**Tavily**（免 key，设了 `TAVILY_API_KEY`
  则走带 key 配额）→ **Bing**（RSS 输出，免 key 免配置）→ **Sogou**（通用兜底），
  另有 **GitHub / arXiv / StackExchange / Hacker News** 四个领域引擎在查询强信号
  时优先参与——全部免配置
...
- 设置卡「搜索结果条数」（1-8，默认 5）：单次搜索返回的来源上限，取 seam 请求量
  与该上限的较小值——条数越多 AI 上下文消耗越大；保存后即时生效（无需重启）
...
安装最新 release 版本（推荐，从仓库 tags 选取最高 release tag）：
